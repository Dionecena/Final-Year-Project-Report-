...
const Sidebar: React.FC = () => {
  ...
  useEffect(() => {
    const fetchUnread = async () => {
      try {
        const res = await axios.get<{ unread_count: number }>('/api/notifications/unread-count');
  ...
  return (
    <div className="flex flex-col w-64 bg-white border-r border-gray-200 min-h-screen">
      ...
      <div className="p-4 border-t border-gray-200">
        <Link
          to="/app/profile"
          className="flex items-center mb-3 hover:bg-gray-50 rounded-lg p-1 transition-colors"
        >
