...
const HeartbeatECG: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ecgShape = (t: number): number => {
      // Normalized t in [0, 1] -> one heartbeat cycle
      const x = t % 1;
      // P wave
    ...
    const updateECGLine = (offset: number) => {
      for (let i = 0; i < totalPoints; i++) {
        ...
        const x = (t - 0.5) * 10; // spread across -5 to 5
    ...
    // Glow line (slightly thicker feel via second pass)
    ...
    const animate = () => {
      ...
      // Move tracking dot
      const dotIdx = Math.floor((t * 80) % totalPoints);
  ...
  return (
    <section className="relative h-[420px] md:h-[420px] h-[300px] overflow-hidden bg-[#030308]">
      <canvas ref={canvasRef} className="w-full h-full block" />
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 text-center z-10 pointer-events-none">
        <h3 className="text-xl font-bold bg-gradient-to-r from-blue-400 via-purple-400 to-blue-400 bg-clip-text text-transparent tracking-tight mb-1">
          Au coeur de votre sante
        </h3>
        <p className="text-sm text-slate-400">Une technologie au service du vivant</p>
      </div>
    </section>
  );
};
